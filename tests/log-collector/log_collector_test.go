...
import (
	...
	v1 "github.com/trilioData/tvk-plugins/api/v1"
	controllerHelpers "github.com/trilioData/tvk-plugins/controllers/helpers"
	restoreController "github.com/trilioData/tvk-plugins/controllers/restore"
	"github.com/trilioData/tvk-plugins/internal"
	"github.com/trilioData/tvk-plugins/internal/helpers"
	"github.com/trilioData/tvk-plugins/internal/utils"
	"github.com/trilioData/tvk-plugins/internal/utils/shell"
	"github.com/trilioData/tvk-plugins/tests/integration/common"
	"github.com/trilioData/tvk-plugins/tests/tools/logprinter"
)
...
var _ = Describe("Log Collector Tests", func() {
	...
	Context("When a restore of backed up application is in available", func() {
		...
		JustAfterEach(func() {
			...
			common.WaitForRestoreToDelete(KubeAccessor, sampleRestoreName, namespace)
		})
	...
	Context("When Trilio Application is in failed state", func() {
		...
		JustBeforeEach(func() {
			...
			for envIndex := range envs {
				env := envs[envIndex]
				if env.Name == internal.DataStoreAttacherImage {
		...
		JustAfterEach(func() {
			...
			controlPlane.Spec.Template.Spec.Containers[0].Env = append(envs,
				corev1.EnvVar{Name: internal.DataStoreAttacherImage, Value: dataStoreAttacherImage})
			err = KubeAccessor.UpdateDeployment(controlPlaneDeploymentKey.Namespace, controlPlane)
			Expect(err).ShouldNot(HaveOccurred())
		})
		...
		It("Should collect Trilio Application Pod yaml-logs", func() {
			...
			targetNS := types.NamespacedName{Name: sampleTargetName + internal.GenerateRandomString(6, true), Namespace: namespace}
	...
	Context("When incorrect arguments passed to log collector command", func() {
		...
		It("Should fail if invalid namespace passed to log collector", func() {
			...
			flags := "--namespaces " + internal.GenerateRandomString(6, true)
...
func getJobYaml(fileName, namespace, job string) *unstructured.Unstructured {
	...
	jobStruct.SetGroupVersionKind(batchv1.SchemeGroupVersion.WithKind(internal.JobKind))
...
func isRestoreResource(job *unstructured.Unstructured, restore *v1.Restore) bool {
	...
	if labels[internal.ControllerOwnerUID] == string(restore.UID) &&
		annotations[internal.ControllerOwnerName] == restore.Name &&
		annotations[internal.ControllerOwnerNamespace] == restore.Namespace {
		return true
	}
	return false
}
...
func getRestoreJobYamls(fileName string, restore *v1.Restore, nRes map[string][]string) (validation string,
	...
	for _, jobName := range nsJobs {
		...
		if isRestoreResource(job, restore) {
			annotations := job.GetAnnotations()
			if annotations[internal.Operation] == internal.MetadataRestoreValidationOperation {
				validation = jobName
			} else if annotations[internal.Operation] == internal.MetadataRestoreOperation {
				metadataRestore = jobName
			} else if annotations[internal.Operation] == internal.DataRestoreOperation {
				dataRestoreMap[annotations[internal.RestorePVCName]] = jobName
			}
		}
	}
...
func getBackupJobYamls(fileName, backupNamespace string, backup *v1.Backup, nRes map[string][]string) (snapshotter string,
	...
	for _, jobName := range nsJobs {
		...
		if metav1.IsControlledBy(job, backup) {
			annotations := job.GetAnnotations()
			if annotations[internal.Operation] == internal.SnapshotterOperation {
				snapshotter = jobName
			} else if annotations[internal.Operation] == internal.RetentionOperation {
				retention = jobName
			} else if annotations[internal.Operation] == internal.MetadataUploadOperation {
				metadataUpload = jobName
			} else if annotations[internal.Operation] == internal.DataUploadOperation {
				datauploadMap[annotations[internal.UploadPVCName]] = jobName
			}
		}
	}
...
func verifyBackupVolumeSnapshot(dataSnapshots []helpers.ApplicationDataSnapshot, volumeSnapshot []string) {
...
func verifyRestoreLogs(restoreName, fileName string, cRes map[string][]string, nRes map[string]map[string][]string) {
	...
	restore, err := KubeAccessor.GetRestore(restoreName, namespace)
	Expect(err).Should(BeNil())
	verifyResourceYaml(cRes, nRes, fileName, internal.RestoreKind, restore)
	...
	if restore.Spec.Source.Backup != nil {
		...
		Expect(err).Should(BeNil())
		verifyResourceYaml(cRes, nRes, fileName, internal.BackupKind, backup)
	}
...
func verifyBackupLogs(backupName, fileName string, cRes map[string][]string, nRes map[string]map[string][]string) {
	...
	verifyResourceYaml(cRes, nRes, fileName, internal.BackupplanKind, backupPlan)
	...
	dataComponents, totalDataComponents := helpers.GetBackupDataComponents(backup.Status.Snapshot, false)
...
func createApplication(appCRFilePath, applicationName string) {
	...
	Expect(common.SetBackupPlanStatus(KubeAccessor, applicationName, namespace, v1.Available)).ShouldNot(HaveOccurred())
...
func verifyTrilioResources(cRes map[string][]string, nRes map[string]map[string][]string, fileName string) {
	...
	verifyResourceYaml(cRes, nRes, fileName, internal.PodKind, ctrlPlane, webhook, exporter)
}
...
func verifyTargetWebAndBackendResources(cRes map[string][]string, nRes map[string]map[string][]string, fileName string) {
	...
	verifyResourceYaml(cRes, nRes, fileName, internal.PodKind, targetBrowserPod, webPod, backendPod)
	verifyResourceYaml(cRes, nRes, fileName, internal.ServiceKind, targetBrowserSvc, webSvc, backendSvc)
	verifyResourceYaml(cRes, nRes, fileName, internal.DeploymentKind, targetBrowserDeploy, webDeploy, backendDeploy)
}
...
func getRetentionJobAnnotations(backup *v1.Backup) map[string]string {
	return map[string]string{
		internal.ControllerOwnerName:      backup.Name,
		internal.ControllerOwnerNamespace: backup.Namespace,
		internal.Operation:                internal.RetentionOperation,
	}
}
...
func getMetadataJobAnnotations(restore *v1.Restore) map[string]string {
	return map[string]string{
		...
		internal.Operation:                internal.MetadataRestoreOperation,
	}
}
...
func getBackupJobTemplate(backup *v1.Backup, annotations map[string]string, isSuccessful bool, isDelayed bool) *batchv1.Job {
	...
	container := controllerHelpers.GetContainer("container", utils.AlpineImage, command, false,
	...
	job := controllerHelpers.GetJob(backup.Name, backup.Namespace, container, []corev1.Volume{},
		controllerHelpers.GetAuthResourceName(backup.UID, internal.BackupKind))
...
func getRestoreJobTemplate(restore *v1.Restore, annotations map[string]string, isSuccessful bool, isDelayed bool) *batchv1.Job {
	...
	job := controllerHelpers.GetJob(restore.Name, restore.Spec.RestoreNamespace, container, []corev1.Volume{},
		controllerHelpers.GetAuthResourceName(restore.UID, internal.RestoreKind))
	job.SetAnnotations(annotations)
	ownerLabels := restoreController.GetRestoreJobLabels(restore)
