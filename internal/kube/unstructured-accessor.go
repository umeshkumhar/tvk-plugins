...
// TODO: make this function associated to accessor & don't take ctx & cl as i/p, but only when accessor is used in job controller
func GetCRDByGroupKind(ctx context.Context, cl client.Client, gk schema.GroupKind) (*unstructured.Unstructured, error) {
	var (
		result *unstructured.Unstructured
		err    error
	)

	v1CRDList := unstructured.UnstructuredList{}
	v1CRDList.SetGroupVersionKind(schema.GroupVersionKind{Group: apiextensions.GroupName, Version: internal.CrdVersionV1,
		Kind: strings.Join([]string{internal.CRDKind, "List"}, "")})

	result, err = getCRDByGK(ctx, cl, v1CRDList, gk)
	if err != nil {
		return nil, err
	}
...
func getCRDByGK(ctx context.Context, cl client.Client, crd unstructured.UnstructuredList,
	gk schema.GroupKind) (*unstructured.Unstructured, error) {
	err := cl.List(ctx, &crd)
	if err != nil {
		log.Error(err, "Error while getting the crd list, ", "kind: ", gk.Kind)
		return nil, err
	}

	for i := range crd.Items {
		c := crd.Items[i]
		val, found, nErr := unstructured.NestedString(c.Object, "spec", "group")
		if !found || nErr != nil || val != gk.Group {
			continue
		}
		// group matched, now check for kind
		val, found, nErr = unstructured.NestedString(c.Object, "spec", "names", "kind")
		if !found || nErr != nil || val != gk.Kind {
			continue
		}
		// group and kind matched, add it to the crdCompMeta
		log.Infof("crd found, %s", c.GetName())
		return &c, nil
	}
	return nil, fmt.Errorf("not able to find the CRD with the given group kind by CRD version: %s",
		crd.GroupVersionKind().Version)
}

// TODO: make this function associated to accessor & don't take ctx & cl as i/p, but only when accessor is used in helm internal library
func UnstructuredListMarshalJSON(ctx context.Context, cl client.Client, gvk schema.GroupVersionKind,
	opts ...client.ListOption) ([]string, error) {
	var (
		result []string
