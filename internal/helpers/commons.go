...
func GetSnapshotCrdVersion(ctx context.Context, cl client.Client, logger logr.Logger) string {
	lgr := logger.WithValues("function", "GetSnapshotCrdVersion")
	lgr.Info("fetching VolumeSnapshot crd to get the stored version")
	defaultVersion := internal.SnapshotVersionV1alpha1

	// Trying with crd apiextention version v1beta1
	resCrd, gErr := kube.GetCRDByGroupKind(ctx, cl, schema.GroupKind{Group: internal.SnapshotGroup, Kind: internal.VolumeSnapshotKind})
	if gErr != nil {
		lgr.Error(gErr, "")
		return defaultVersion
	}

	version, vFound, vErr := unstructured.NestedString(resCrd.Object, "spec", "version")
	if !vFound || vErr != nil {
		lgr.Info("spec.version is not found in crd, fetching all the versions and checking which one is storage=true")
		versions, found, nErr := unstructured.NestedSlice(resCrd.Object, "spec", "versions")
		if nErr != nil {
			lgr.Error(nErr, "")
			return defaultVersion
		}
		if !found {
			lgr.Error(errors.New("error while getting the version of VolumeSnapshot CRD"), "")
			return defaultVersion
		}
